"""Native Mojo GPU kernels for the 3D cube automaton (B6/S567).

The volume is toroidal in all three dimensions. Each thread updates one
cube per generation using 26-neighbor counts.
"""
...
alias GPU_STRIDE: Int = ((cube_WIDTH + 63) // 64) * 64
alias GPU_LAYER_STRIDE: Int = GPU_STRIDE * cube_HEIGHT
alias GRID_SIZE: Int = GPU_LAYER_STRIDE * cube_DEPTH
...
@always_inline
fn _apply_cube_rules(current: Int, neighbors: Int) -> UInt8:
...
fn cube_generation_kernel(
    ...
    write_grid[idx] = _apply_cube_rules(current, neighbors)
