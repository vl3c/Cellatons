"""Voxel (3D) cellular automaton viewer (GPU-only)."""

from python import Python, PythonObject
from voxel.grid import Grid, VOXEL_WIDTH, VOXEL_HEIGHT, VOXEL_DEPTH, INITIAL_DENSITY
from voxel.renderer import RendererConfig, init_display, PythonModuleRenderer
...
fn _print_banner():
    print("=" * 60)
    print("Voxel Automaton (3D, GPU-only)")
    print("=" * 60)
    print("Grid:", VOXEL_WIDTH, "x", VOXEL_HEIGHT, "x", VOXEL_DEPTH)
...
fn run_viewer() raises:
    ...
    if not has_gpu:
        print("No GPU detected. Voxel viewer requires GPU acceleration.")
        return
    ...
    var config = init_display("Voxel Automaton (GPU)")
