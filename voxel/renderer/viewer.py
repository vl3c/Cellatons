"""Python-side renderer for the 3D voxel automaton.

Renders a rotating voxel cube with:
- Black background
- White live voxels
...
GRID_STEP = 8   # draw a lattice line every GRID_STEP voxels
MAX_VISIBLE_VOXELS = 60_000  # cap rendered voxels for performance
...
def _render_grid_lines(pygame, screen, lines: np.ndarray, rot: np.ndarray, display_w: int, display_h: int):
    ...
    scale = min(display_w, display_h) * 0.6  # same scale as voxels
...
def render_frame(
    ...
    # Draw grid lines ON TOP of voxels
