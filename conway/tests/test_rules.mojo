"""Tests for Conway's Game of Life rules (B3/S23)."""
...
fn _apply_conway_rules_single_cell(grid: Grid, row: Int, col: Int) -> Int:
    """Apply Conway's rules to a single cell and return new state."""
...
fn test_birth_with_3_neighbors() raises:
    ...
    var result = _apply_conway_rules_single_cell(grid, 5, 5)
...
fn test_still_life_block() raises:
    ...
    assert_equal(_apply_conway_rules_single_cell(grid, 4, 4), 1)
    assert_equal(_apply_conway_rules_single_cell(grid, 4, 5), 1)
    assert_equal(_apply_conway_rules_single_cell(grid, 5, 4), 1)
    assert_equal(_apply_conway_rules_single_cell(grid, 5, 5), 1)
...
fn test_blinker_horizontal_to_vertical() raises:
    ...
    assert_equal(_apply_conway_rules_single_cell(grid, 5, 4), 0)
    assert_equal(_apply_conway_rules_single_cell(grid, 5, 6), 0)
    
    # Above and below center should be born (3 neighbors each)
    assert_equal(_apply_conway_rules_single_cell(grid, 4, 5), 1)
    assert_equal(_apply_conway_rules_single_cell(grid, 6, 5), 1)
