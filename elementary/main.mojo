...
fn benchmark_native_gpu_only(rule_container: RuleContainer) raises:
    """Benchmark native GPU without any CPU grid allocation.
    
    This function runs GPU computation directly without creating CPU-side
    Grid objects. Use this for pure GPU benchmarking or when CPU memory
    is limited but GPU memory is sufficient.
    """
    var py_time = Python.import_module("time")
    var py_builtins = Python.import_module("builtins")
    var py_operator = Python.import_module("operator")
    ...
    for rule in rule_container.rules:
        # No CPU grid allocation! Direct GPU-only benchmark
    ...
    if total_runs > 0:
        ...
        var elapsed = py_builtins.format(end - start, ".3f")
        print("Benchmarked", len(rule_container.rules), "rules (GPU-only, no CPU grid) in", elapsed, "seconds (", total_gpu_str, "s on native GPU)", sep=" ")
...
fn main() raises:
    ...
    benchmark_native_gpu_only(rule_container)  # GPU-only benchmark (no CPU grid allocation)
